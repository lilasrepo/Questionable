...
internal sealed class QuestInfo : IQuestInfo
{
    ...
    {
        // MUST RE-APPLY (was pinned as Model/QuestInfo.cs until upstream moved this type to
        // Questionable.Domain; the old pin went stale and the fix came back out with it).
        // TempQuest is a custom 7.5-layout sheet: its column hash does not match the TC data set
        // and reading it CRASHES PLUGIN LOAD -- compiles clean, fails at startup. Stock ExcelQuest
        // already exposes every column used below, so use it directly.
            ...
            .Select(x =>
            {
                var item = Svc.Data.GetExcelSheet<Item>()
                    .Where(item => item.ItemAction.Value is var itemAction &&
                        itemAction.Type == 3357 && // triple triad card use action
