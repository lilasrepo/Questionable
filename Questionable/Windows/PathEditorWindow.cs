...
internal sealed class PathEditorWindow : LWindow
{
    ...
    public override void DrawContent()
    {
        ...
        using (ImRaii.IEndObject tree = ImRaii.Child("PathEditorTree",
        ...
        using ImRaii.IEndObject detail = ImRaii.Child("PathEditorDetail", Vector2.Zero, border: false);
        if (detail)
        {
    ...
    private void DrawSequenceTree()
    {
        ...
        for (int sequenceIndex = 0; sequenceIndex < root.QuestSequence.Count; sequenceIndex++)
        {
            ...
            using ImRaii.Id id = ImRaii.PushId(sequenceIndex);
            using ImRaii.IEndObject node = ImRaii.TreeNode(
