...
internal sealed class QuestValidationWindow : LWindow
{
    ...
    public override void DrawContent()
    {
        ...
        using ImRaii.IEndObject table = ImRaii.Table("QuestSelection", 6, ImGuiTableFlags.Borders | ImGuiTableFlags.ScrollY);
        if (!table)
        {
        ...
        foreach (ValidationIssue validationIssue in _questValidator.Issues.OrderBy(i => i.Description, StringComparer.OrdinalIgnoreCase))
        {
            ...
            if (ImGui.TableNextColumn())
            {
                using (_ = _pluginInterface.UiBuilder.IconFontFixedWidthHandle.Push())
                {
                    ...
                    {
                        using ImRaii.Color color = ImRaii.PushColor(ImGuiCol.Text, QstTheme.Danger);
                        ImGui.TextUnformatted(FontAwesomeIcon.ExclamationTriangle.ToIconString());
                    }
                    else
                    {
                        using ImRaii.Color color = ImRaii.PushColor(ImGuiCol.Text, QstTheme.Info);
